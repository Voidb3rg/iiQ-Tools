...
function inject_hidden_iframe(){
    var hotelid = document.querySelector("#nav-right > div.userbox.dropdown.hidden-xs > div > li:nth-child(2) > a").pathname.split('/')[3]
    if (hotelid !== null){
        $('<iframe id="stamm" style="display: block; width:1000px">').attr('src', '/admin/hotels/' + hotelid + '/edit').appendTo('body')}
}
var hotelid = document.querySelector("#nav-right > div.userbox.dropdown.hidden-xs > div > li:nth-child(2) > a").pathname.split('/')[3];
if (hotelid !== null){
$('<iframe id="stamm" style="display: block; width:1000px">').attr('src', '/admin/hotels/' + hotelid + '/edit').appendTo('body');}
...
function get_hotel_email() {
  if ($('#hotel_contact_request_email', $('#stamm').contents()).val()){
    console.log("has contact mail")
  }
  else if ($('#hotel_email', $('#stamm').contents()).val()){
    console.log("has hotel mail")
  }
  }
